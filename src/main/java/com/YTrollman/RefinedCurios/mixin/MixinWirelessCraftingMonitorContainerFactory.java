...
public class MixinWirelessCraftingMonitorContainerFactory {
    ...
    public CraftingMonitorContainer create(int windowId, PlayerInventory inv, PacketBuffer data) {
        ...
        if(CuriosApi.getCuriosHelper().findEquippedCurio(RSItems.WIRELESS_CRAFTING_MONITOR.get(), inv.player).isPresent()) {
            WirelessCraftingMonitor wirelessCraftingMonitor = new WirelessCraftingMonitor(CuriosApi.getCuriosHelper().findEquippedCurio(RSItems.WIRELESS_CRAFTING_MONITOR.get(), inv.player).get().getRight(), null, slotId);
            ...
            WirelessCraftingMonitor wirelessCraftingMonitor = new WirelessCraftingMonitor(CuriosApi.getCuriosHelper().findEquippedCurio(RSItems.CREATIVE_WIRELESS_CRAFTING_MONITOR.get(), inv.player).get().getRight(), null, slotId);
            return new CraftingMonitorContainer(RSContainers.WIRELESS_CRAFTING_MONITOR, wirelessCraftingMonitor, null, inv.player, windowId);
        }
        else
        {
